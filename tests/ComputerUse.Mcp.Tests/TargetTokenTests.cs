...
public sealed class TargetTokenTests
{
    ...
    [Fact]
    public void SameIdentity_ReusesIssuedToken()
    {
        var world = new FakeWorld();
        world.Processes[10] = new FakeProcess { Pid = 10, CreateTimeUtc = 1000 };
        world.Windows[1] = new FakeWindow { Hwnd = 1, Pid = 10, ClassName = "Notepad" };
        ...
        var second = tokens.Issue(1, 10, 1000, "Notepad");
        Assert.Equal(first, second);
        Assert.Equal(10u, tokens.RequireValid(first, world, world).Pid);
    }

    [Fact]
    public void RevokedToken_IsNotReused()
    {
        var tokens = new TargetTokenService();
        var first = tokens.Issue(1, 10, 1000, "Notepad");
        tokens.Revoke(first);
    ...
    [Fact]
    public void IdentityChange_IssuesNewToken()
    {
        var tokens = new TargetTokenService();
        var first = tokens.Issue(1, 10, 1000, "Notepad");
        var second = tokens.Issue(1, 11, 2000, "Notepad");
        Assert.NotEqual(first, second);
    }

    [Fact]
    public void MatchesLive_RejectsPidReuse()
    {
        var world = new FakeWorld();
        world.Processes[10] = new FakeProcess { Pid = 10, CreateTimeUtc = 1000 };
        world.Windows[1] = new FakeWindow { Hwnd = 1, Pid = 10, ClassName = "Notepad" };
        var payload = new TargetTokenPayload
        {
            Hwnd = 1,
            Pid = 10,
            CreateTimeUtc = 1000,
        ...
        Assert.True(TargetTokenService.MatchesLive(payload, world, world));
        world.Windows[1].Pid = 11;
        world.Processes[11] = new FakeProcess { Pid = 11, CreateTimeUtc = 2000 };
        Assert.False(TargetTokenService.MatchesLive(payload, world, world));
    }
